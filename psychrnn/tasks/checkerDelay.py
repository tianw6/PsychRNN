...
Tian Wang, Nov 5th, 2023 (training failed)
...
class Checkerboard2AFC(Task):
    ...
    input 1: left and right target: only 0 and 1 (1 represents targets showed up)
    input 2: color coherence (continuous)
        1: completely red
        -1: completely green

    output 1: red output (either 0 or 1)
    output 2: green output (either 0 or 1) 
    ...
    def generate_trial_params(self, batch, trial):
        ...
        params["checkerDur"] = self.checkerDur
        params["checker_onset"] = np.random.randint(self.checker_onset[0], self.checker_onset[1])
        params["delay"] = np.random.randint(self.delay[0], self.delay[1])
    ...
    def trial_function(self, t, params):
        ...
        if t > delay + checker_onset + checkerDur:

            x_t[0] = 1 


        # checker_onset ()
        if t > checker_onset and t < checkerDur + checker_onset:
            x_t[1:] = (params["noise"] ** 2) * np.sqrt(self.dt) * np.random.randn(1)
            x_t[1] += coherence
        ...
        if t > delay + checkerDur + checker_onset:
            y_t[correct_side] = self.hi
            y_t[abs(correct_side - 1)] = self.lo


        mask_t = np.ones(self.N_out)
        # if (t > delay + checkerDur + checker_onset) and (t < delay + checkerDur + checker_onset + accumulation_mask):
        if (t < delay + checkerDur + checker_onset + accumulation_mask):
