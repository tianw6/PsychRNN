# add an input mask: when training RNN, set to all 1; then assign more value to left modulated units
...
class Basic2(Basic):
    ...
    def __init__(self, params, input_mask):
       ...
       #  return new_state
        ...
        new_state = (
            ...
            * (
                ...
                + tf.matmul(
                    rnn_in, (self.get_effective_W_in()*input_mask), transpose_b=True, name="2"
                )
                + self.b_rec
            )
            + tf.sqrt(2.0 * self.alpha * self.rec_noise * self.rec_noise)
